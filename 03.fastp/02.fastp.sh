...
INDIR=/kuhpc/work/colella/ben/009.RBV.WGS/20250325_NX2K-P4-PE150_BJW-RBV-WGS-2/20250325_NX2K-P4-PE150_BJW-RBV-WGS-2-Colella_FASTQ
OUTDIR=/kuhpc/scratch/bi/b686w673/009.RBV.WGS/03.fastp
#mkdir $OUTDIR

# get all fastq files
fastqs=`ls $INDIR/FN*.fastq.gz`

# get all unique identifiers
FNs=`basename -a $fastqs | sed 's/_S.*//' | sort | uniq`

# figure out which FN of this iteration of the array
FN=$(echo "$FNs" | sed -n "${SLURM_ARRAY_TASK_ID}p")
echo $FN

# define input read files and output read files
READ1=$INDIR/$FN*R1*.fastq.gz
READ2=$INDIR/$FN*R2*.fastq.gz
OUT1=$OUTDIR/$FN.R1.fastq.gz
OUT2=$OUTDIR/$FN.R2.fastq.gz
UNPAIRED1=$OUTDIR/$FN.U1.fastq.gz
UNPAIRED2=$OUTDIR/$FN.U2.fastq.gz
