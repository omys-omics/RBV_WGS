...
#SBATCH --nodes=1
#SBATCH --ntasks=24
#SBATCH --output=outputs/out.%j.CLRU.snps.txt
...
#find the bam files
bams=`ls /kuhpc/work/colella/ben/009.RBV.WGS/06.alignments.CLRU/01.alignments/MSB*.bam`
echo $bams
for bam in $bams
do
  KU=`basename $bam`
...
bcftools mpileup --threads 96 -a DP,AD -q 30 -Q 30 -Ou -f $ref $bams | bcftools call -f GQ -mv -V indels -Ou -o $out_dir/01.raw.snps.CLRU.bcf
#-G, --read-groups FILE
